...
void MaybePushBack(std::vector<EventType>* output, std::optional<EventType> event_type) {
...
uint64_t PhysicallyIndexed::EnsureLogicalKey(NativeEvent& native_event, bool force_update) {
...
uint64_t LogicallyIndexed::EnsurePhysicalKey(NativeEvent& native_event, bool force_update) {
  const uint64_t logical_key = native_event.logical_key();
  if (!force_update) {
...
void PressStateTracker::RequireTextKeyState(std::vector<FlutterKeyEvent>* output,
...
void PressStateTracker::RequireModifierKeyState(std::vector<FlutterKeyEvent>* output,
                              NativeEvent& source_event,
...
std::vector<EventType> PressStateTracker::ModelTextKeyEvent(uint64_t physical_key,
                                          std::optional<bool> require_pressed_before,
                                          bool require_pressed_after) {
...
void PressStateTracker::ModelModifierState(std::vector<ModifierStateChange>* output,
...
std::optional<EventType> PressStateTracker::EnsurePressed(StateMap::iterator current, bool require_pressed) {
  ...
  void LockStateTracker::RequireState(std::vector<FlutterKeyEvent>* output,
                    NativeEvent& source_event,
  ...
  std::vector<LockStateTracker::StateChange> LockStateTracker::ModelState(uint64_t logical_key,
                                      std::optional<LockState> require_primary_state,
                                      LockState require_after_cleanup) {
