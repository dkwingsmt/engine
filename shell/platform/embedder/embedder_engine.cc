...
bool EmbedderEngine::DispatchKeyMessagePacket(
    std::unique_ptr<flutter::KeyMessagePacket> packet,
  ...
  platform_view->DispatchKeyMessagePacket(std::move(packet), std::move(callback));
  return true;
}
